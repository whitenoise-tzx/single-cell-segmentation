...
def get_iou(mask_name,predict):
    image_mask = cv2.imread(mask_name,cv2.COLOR_BGR2GRAY)#0
    ...
    #Iou = IOUMetric(3)  #2表示类别，肝脏类+背景类
    #Iou.add_batch(predict, image_mask)
    #a, b, c, d, e= Iou.evaluate()
    #print('%s:acc, acc_cls, iu, mean_iu, fwavacc=%f,%f,%f,%f,%f' % (mask_name, a, b, c, d, e))
...
def get_dice(mask_name,predict):
    image_mask = cv2.imread(mask_name,cv2.COLOR_BGR2GRAY)
