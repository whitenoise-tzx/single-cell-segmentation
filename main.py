...
def val(model,best_iou,val_dataloaders):
    ...
    with torch.no_grad():
        ...
        for x, _,pic,mask in val_dataloaders:
            ...
            plt.imshow(pred)
            plt.subplot(1, 3, 3)
            plt.imshow(torch.squeeze(x).cpu().numpy())
            plt.show()
            '''
...
def test(val_dataloaders,save_predict=False):
    ...
    if save_predict ==True:
        ...
        dir = os.path.join(r'./saved_predict', 'transfer',str(args.arch), str(args.batch_size), str(args.epoch), str(args.dataset))
    ...
    with torch.no_grad():
        ...
        print('aver_pixel_accuracy=%f, Miou=%f, aver_boundary_iou=%f, aver_avgHausdorff=%f' % (aver_pixel_accuracy, aver_iou, aver_boundary_iou, aver_avgHausdorff))
        logging.info('aver_pixel_accuracy=%f, Miou=%f, aver_boundary_iou=%f, aver_avgHausdorff=%f' % (aver_pixel_accuracy, aver_iou, aver_boundary_iou, aver_avgHausdorff))
...
if __name__ =="__main__":
    ...
    print('**************************')
    model = getModel(args)
    #model.load_state_dict(torch.load('./saved_model/unet++_4_HK2_DIC_41.pth'))  # 再加载网络的参数
